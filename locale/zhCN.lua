...
L.None = "无"
L.DisplayAccurateBattlePets = "启用公会银行和邮箱中的精确战斗宠物统计。|cFFDF2B2B（可能导致卡顿）|r |cff3587ff[参见 BagSync 常见问题]|r"
L.DisplaySortCurrencyByExpansionFirst = "在 BagSync 货币窗口中优先按内容更新排序，而非字母顺序。"
...
L.ExtTipNoticeText = "|cFFFFFFFF暴雪的|cFFFF0000秘密值|r|cFFFFFFFF可能会阻止外部提示框定位。BagSync 添加了保护措施，但如果找不到安全的提示框，计数将回退到 |cFFFF0000游戏提示框|r|cFFFFFFFF。详细信息请参阅有关|cFFFF0000“秘密值”|r|cFFFFFFFF的常见问题解答。|r"
L.ConfigTracking = "追踪"
...
L.FAQ_Question_7_p1 = [[
暴雪不会在公会银行或邮箱中使用正确的物品ID或物种ID存储战斗宠物。实际上，战斗宠物在公会银行和邮箱中存储为 |cFF99CC33[宠物笼]|r，物品ID为 |cFF99CC3382800|r。这使得插件作者难以获取特定战斗宠物的数据。您可以在公会银行交易日志中自行查看，会注意到战斗宠物被存储为 |cFF99CC33[宠物笼]|r。如果您从公会银行链接一个，它也会显示为 |cFF99CC33[宠物笼]|r。为了解决这个问题，可以使用两种方法。第一种方法是将战斗宠物分配到提示框，然后从那里获取物种ID。这需要服务器响应魔兽客户端，如果公会银行中有大量战斗宠物，可能会导致严重卡顿。第二种方法使用战斗宠物的图标纹理来尝试查找物种ID。这有时不准确，因为某些战斗宠物共享相同的图标纹理。例如：毒黏怪与翡翠软泥怪共享相同的图标纹理。启用此选项将强制使用提示框扫描方法以尽可能精确，但这可能导致卡顿。|cFFDF2B2B在暴雪提供更多可用数据之前，无法解决此问题。|r
]]
